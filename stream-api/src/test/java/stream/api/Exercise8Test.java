...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
         ...
         */
        List<String> itemListOnSale = shopStream.flatMap(customer -> customer.getItemList().stream()).map(Item::getName).collect(Collectors.toList());
        Set<String> itemSetNotOnSale = customerStream.flatMap(shop -> shop.getWantToBuy().stream()).map(Item::getName)
                .filter(itemName -> itemListOnSale.stream().noneMatch(itemName::equals)).collect(Collectors.toSet());
